"""Burger Menu Combo Manager Program, to manage combo meals.
Practice task for 12DTC
By Ethan Wong
"""

from easygui import *

MAX_ITEMS_IN_COMBO = 3  # a burger, side, and drink
...
def instructions():
    ...
    msg3 = f"When creating a NEW combo, you will be limited to" \
           f" {MAX_ITEMS_IN_COMBO} items for the purposes of this program;" \
           f" most likely a burger, side, and drink."
...
def add(menu, combo, combo_name):
    ...
    while True:
        choice = buttonbox(f"*{combo_name}* details have been printed to "
                           f"console.\n\n"
...
def get_new_price(msg, title):
    try:
        new_price = enterbox(msg, title).strip("$")  # allows input to have $
...
def print_combo(combo, combo_name, solo=True):
    ...
    # Display Results
    print(f"* {combo_name.upper() + ' *':<20} = ${total_price:.2f}")
    for item in combo:
        print(f"  {item[0] + ' ':.<20} - ${item[1]:.2f}")

    print()

    if solo:  # if it's only the individual combo being printed
        print("####")
        print()
...
def print_menu(menu):
    ...
    for combo in menu:
        print_combo(menu[combo], combo, False)
